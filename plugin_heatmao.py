...
class VictoryHeatmapPlugin(QWidget):
    ...
    def init_ui(self):
        ...
        # 定义一个通用的“统计格子”工厂函数，把字和逻辑都改大
        def add_box(t, v):
            w = QVBoxLayout()
            # 标题文字：加大一倍 (从 14 改为 28)
            l1 = QLabel(t); l1.setStyleSheet("color: #666; font-size: 28px;"); l1.setFont(QFont("微软雅黑", 28, QFont.Bold)) # 28px
            l1.setAlignment(Qt.AlignCenter) # 居中

            # 时间文字：加大一倍 (从 24 改为 48)
            l2 = QLabel(f"{v} min"); l2.setStyleSheet("color: #1b5e20; font-size: 48px; font-weight: bold;"); l2.setFont(QFont("Segoe UI", 48, QFont.Bold)) # 48px
            l2.setAlignment(Qt.AlignCenter) # 居中
            
            w.addWidget(l1)
            w.addWidget(l2)
            s_layout.addLayout(w)
            
            # 在每两个格子之间增加一条精致的垂直分隔线
            line = QFrame()
        ...
        # 移除最后一个 add_box 的分隔线 (代码优化)
        s_layout.itemAt(s_layout.count()-1).widget().deleteLater() 
        add_box(f"第 {self.stats['week_num']} 周", self.stats['week']) # 标注第几周
        s_layout.itemAt(s_layout.count()-1).widget().deleteLater() # 移除多余线

        layout.addSpacing(30); layout.addWidget(stats_box); layout.addStretch(1)

        # 4. 置底总计文字
        total_all = sum(self.data_map.values())
        footer = QLabel(f"成就达成：{total_all} 分钟")
        footer.setObjectName("FooterLabel") # 对应 QSS #FooterLabel
        footer.setFont(QFont("微软雅黑", 32, QFont.Bold)) # 32px加粗
        footer.setStyleSheet("color: #1b5e20; border-top: 2px solid #eee; padding-top: 15px;") # 顶边框线
        footer.setAlignment(Qt.AlignCenter); layout.addWidget(footer)

# ==============================================================================
# 傻瓜式操作最后一步：启动代码块 (必须顶格，不能缩进)
# ==============================================================================
